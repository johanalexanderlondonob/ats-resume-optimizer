...
function educationsFrom(resumeBase: ResumeBaseResponseDTO): EducationRow[] {
    return resumeBase.educations.map(education => ({
        key: genId(),
        type: education.type,
        ...
        startDate: toMonthInput(education.startDate),
        finishDate: toMonthInput(education.finishDate),
        isCurrent: education.isCurrent,
    }));
}

function experiencesFrom(resumeBase: ResumeBaseResponseDTO): ExperienceRow[] {
    return resumeBase.experiences.map(experience => ({
        key: genId(),
...
function projectsFrom(resumeBase: ResumeBaseResponseDTO): ProjectRow[] {
    return resumeBase.projects.map(project => ({
        key: genId(),
        title: project.title,
        role: project.role,
...
function languagesFrom(resumeBase: ResumeBaseResponseDTO): LanguageRow[] {
    return resumeBase.languages.map(language => ({
        key: genId(),
...
function referencesFrom(resumeBase: ResumeBaseResponseDTO): ReferenceRow[] {
    return resumeBase.references.map(reference => ({
        key: genId(),
...
function skillsFrom(resumeBase: ResumeBaseResponseDTO): SkillRow[] {
    return resumeBase.skills.map(skill => ({
        key: genId(),
...
export default function EditResumeBaseForm({ candidateId, resumeBase }: {
    candidateId: string;
    resumeBase: ResumeBaseResponseDTO;
}) {
    const router = useRouter();

    const [profession, setProfession] = useState(resumeBase.profession);
    const [aboutMe, setAboutMe] = useState(resumeBase.aboutMe ?? '');
    const [professionalProfile, setProfessionalProfile] = useState(resumeBase.professionalProfile ?? '');
    const [salaryAspiration, setSalaryAspiration] = useState(String(resumeBase.salaryAspiration ?? ''));
    const [salaryCurrency, setSalaryCurrency] = useState(resumeBase.salaryCurrency || 'COP');

    const [educations, setEducations] = useState<EducationRow[]>(() => educationsFrom(resumeBase));
    const [experiences, setExperiences] = useState<ExperienceRow[]>(() => experiencesFrom(resumeBase));
    const [projects, setProjects] = useState<ProjectRow[]>(() => projectsFrom(resumeBase));
    const [languages, setLanguages] = useState<LanguageRow[]>(() => languagesFrom(resumeBase));
    const [references, setReferences] = useState<ReferenceRow[]>(() => referencesFrom(resumeBase));
    const [skills, setSkills] = useState<SkillRow[]>(() => skillsFrom(resumeBase));
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            if (!response.ok || json?.error) {
                ...
                setMessage({
                    type: 'error',
                    text: json?.error?.message ?? 'No se ha podido actualizar la hoja de vida',
                });
                return;
            }

            setMessage({ type: 'success', text: json.message ?? 'Hoja de vida actualizada satisfactoriamente' });
            router.push(`/candidates/${ candidateId }/resume-base/preview`);
        } catch {
            setMessage({ type: 'error', text: 'No se ha podido actualizar la hoja de vida' });
        } finally {
            setPending(false);
        }
    }
